  ...
  if (screen === "login") return (
    <div style={{ minHeight:"100vh", background:"#F7F4EF", fontFamily:"DM Sans,sans-serif", maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column", position:"relative" }}>
      ...
      <div style={{ flex:1, padding:"24px 24px 40px" }}>
        ...
        <button onClick={async()=>{ if(!email||!email.includes("@")){alert("Please enter a valid email");return;} if(!pass||pass.length<6){alert("Password must be at least 6 characters");return;} const {error}=await signIn(email,pass); if(error){alert(error.message);return;} onLogin(); }} style={{ width:"100%", marginTop:18, padding:"14px 0", background:G, color:"#fff", border:"none", borderRadius:14, fontSize:15, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 16px rgba(61,107,53,.3)" }}>{t.loginBtn}</button>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={async()=>{ await supabase.auth.signInWithOAuth({provider:"google",options:{redirectTo:"https://cohood.nl"}}); }} style={{ flex:1, padding:"11px 0", background:"#fff", border:"1.5px solid #E2D9CC", borderRadius:12, cursor:"pointer", fontSize:13, color:"#2C2416", fontWeight:600, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
