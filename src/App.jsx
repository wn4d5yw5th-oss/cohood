  ...
  if (screen === "register") return (
    <div style={{ minHeight:"100vh", background:"#F7F4EF", fontFamily:"DM Sans,sans-serif", maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column", position:"relative" }}>
      ...
      <div style={{ flex:1, padding:"24px 24px 40px" }}>
        ...
        <button onClick={()=>setScreen("verify-email")} style={{ width:"100%", marginTop:18, padding:"14px 0", background:G, color:"#fff", border:"none", borderRadius:14, fontSize:15, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 16px rgba(61,107,53,.3)" }}>{t.registerBtn}</button>
      ...
      <div style={{ flex:1, padding:"28px 24px 40px" }}>
        ...
        <div style={{ background:"#fff", border:"1px solid #E2D9CC", borderRadius:14, padding:"14px 16px" }}>
          <div style={{ fontSize:13, color:"#6B5E4E", lineHeight:1.7 }}>
            ...
            <button style={{ background:"none", border:"none", color:G, fontWeight:700, cursor:"pointer", fontSize:13, fontFamily:"DM Sans,sans-serif" }}>
    ...
    <div style={{ minHeight:"100vh", background:"#F7F4EF", fontFamily:"DM Sans,sans-serif", maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:"linear-gradient(160deg,#F0EBE1,#E8E0D0)", padding:"40px 32px 28px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:16 }}>
          {[1,2,3].map(s => (
            <div key={s} style={{ flex:1, height:4, borderRadius:2, background:s<=3?G:"#E2D9CC", transition:"all .3s" }}/>
          ))}
        </div>
      ...
      <div style={{ flex:1, padding:"24px 24px 40px" }}>
        <div style={{ display:"flex", justifyContent:"center", marginBottom:20 }}>
          <div style={{ position:"relative" }}>
            <div style={{ width:80, height:80, borderRadius:"50%", background:G, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, fontWeight:700, color:"#fff" }}>
              {name?name[0].toUpperCase():"?"}
            </div>
            <div style={{ position:"absolute", bottom:0, right:0, width:26, height:26, background:G, borderRadius:"50%", border:"2px solid #F7F4EF", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:16 }}>
          <Field icon="user" ph={isNL?"Voornaam":"First name"} val={name} set={setName}/>
          <Field icon="mapPin" ph={isNL?"Jouw buurt (bijv. De Pijp)":"Your neighborhood (e.g. De Pijp)"} val={hood} set={setHood}/>
          <div style={{ background:"#F0EBE1", border:"1.5px solid #E2D9CC", borderRadius:12, padding:"12px 14px" }}>
            <textarea value={bio} onChange={e=>setBio(e.target.value)} placeholder={isNL?"Korte bio (optioneel)...":"Short bio (optional)..."} style={{ width:"100%", border:"none", outline:"none", background:"transparent", fontSize:14, color:"#2C2416", fontFamily:"DM Sans,sans-serif", resize:"none", minHeight:70, boxSizing:"border-box" }}/>
          </div>
        </div>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {(isNL?["Vertaling","Koken","Tuinieren","Vervoer","Reparaties","Kinderopvang"]:["Translation","Cooking","Gardening","Transport","Repairs","Childcare"]).map(o=>(
              <button key={o} onClick={()=>setOffers(prev=>prev.includes(o)?prev.filter(x=>x!==o):[...prev,o])}
                style={{ padding:"7px 14px", borderRadius:20, border:`1.5px solid ${offers.includes(o)?G:"#E2D9CC"}`, background:offers.includes(o)?GL:"#fff", color:offers.includes(o)?G:"#6B5E4E", fontSize:13, fontWeight:600, cursor:"pointer", transition:"all .2s" }}>
                {o}
              </button>
            ))}
          </div>
        ...
        <button onClick={()=>setScreen("onboarding")} style={{ width:"100%", padding:"14px 0", background:G, color:"#fff", border:"none", borderRadius:14, fontSize:15, fontWeight:700, cursor:"pointer" }}>
